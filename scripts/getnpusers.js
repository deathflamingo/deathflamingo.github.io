function createToolForm() {
    ...
    const option3 = createRadioOption('noCredentials', 'Without Credentials');
    ...
    form.appendChild(document.createElement('br'));
    form.appendChild(option3.input);
    form.appendChild(option3.label);
    form.appendChild(document.createElement('br'));
    ...
    option3.input.addEventListener('change', () => updateFields(dynamicFields, 'noCredentials'));
...
function updateFields(container, option) {
    ...
    switch (option) {
        ...
        case 'noCredentials':
            container.appendChild(createField('Domain', 'domain', 'text'));
            container.appendChild(createField('DC IP', 'dcip', 'text'));
            container.appendChild(createField('Users File', 'usersfile', 'text'));
            container.appendChild(createField('Output File', 'outfile', 'text'));
            break;
    }
}
...
function generateCommand(container) {
    ...
    switch (option) {
        ...
        case 'hashes':
            ...
            break;
        case 'noCredentials':
            const usersfile = formData.get('usersfile');
            const outfile = formData.get('outfile');
            command += `/ -dc-ip ${dcip} -usersfile ${usersfile} -outfile ${outfile} -request`;
            break;
    }
    return command;
}
